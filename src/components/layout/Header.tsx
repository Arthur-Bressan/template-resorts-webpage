  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <a href="#" className="flex items-center gap-2.5 group">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-colors duration-300 ${
              scrolled ? "bg-[var(--color-primary)]" : "bg-white/20 backdrop-blur-sm border border-white/30"
            }`}>
              <Leaf className="w-5 h-5 text-white" />
            </div>
            <span className={`font-serif text-lg font-medium tracking-tight transition-colors duration-300 ${
      ...
      >
        ...
        />
        <div
          className={`absolute right-0 top-0 bottom-0 w-80 max-w-[85vw] bg-[var(--color-background)] shadow-xl transition-transform duration-500 ${
            ...
            <nav className="flex flex-col gap-1">
              {navLinks.map((link) => (
                <button
                  key={link.href}
                  onClick={() => handleNavClick(link.href)}
                  className="text-left py-3 px-4 rounded-xl text-[var(--color-text)] font-medium hover:bg-[var(--color-surface)] transition-colors"
                >
                  {link.label}
                </button>
              ))}
