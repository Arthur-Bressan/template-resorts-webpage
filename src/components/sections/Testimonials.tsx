...
export function Testimonials() {
  ...
  return (
    ...
    >
      {/* Background blob */}
      <div
        className="blob w-[400px] h-[400px] bg-[var(--color-accent)] top-20 -left-32"
        style={{ opacity: 0.1 }}
      />
        ...
        <div className="grid md:grid-cols-3 gap-6" data-stagger>
          {testimonials.map((t) => (
            <article
              key={t.id}
              data-stagger-child
              className="group relative bg-[var(--color-background)] rounded-2xl p-8 shadow-md hover:shadow-lg transition-shadow duration-500"
            >
              {/* Quote icon */}
              ...
              <div className="flex items-center gap-3 pt-5 border-t border-[var(--color-border)]">
                <div className="w-11 h-11 rounded-full overflow-hidden bg-[var(--color-surface)] shrink-0">
                  <img
                    src={t.avatar}
                    alt={t.name}
